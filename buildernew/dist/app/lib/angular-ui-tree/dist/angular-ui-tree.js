        ...
        return {
          ...
          link: function (scope, element, attrs, controllersArr) {
            ...
            dragStart = function (e) {
              ...
              //Create drag element.
              $document.find(scope.scrollContainer).append(dragElm);

              //Set drag elements position on screen.
               dragElm.css({
                'left': eventObj.pageX - pos.offsetX + $(body).scrollLeft() -15 -15 + 'px',
                'top': eventObj.pageY - pos.offsetY + $(body).scrollTop() - 100 + 'px'
              });

              
              scope._parentHeight = $(scope.scrollContainer)[0].scrollHeight - 300;
              scope._parentWidth = $(scope.scrollContainer)[0].scrollWidth - 50;
            ...
            dragMove = function (e) {
              ...
              if (dragElm) {
                ...
                // !kore
                if($(eventObj.target) && $(eventObj.target).scope() && $(eventObj.target).scope()['block']){
                   var _this = $(eventObj.target).closest('.blockContent');
                   _this.hover(function () {  
                    $(_this).addClass('nodehovered');
                  }, 
                  function () {
                    $(_this).removeClass('nodehovered');
                  })
              }
                ...
                //Updating element being moved css.
                 dragElm.css({
                  'left': leftElmPos + $(body).scrollLeft() - 30 + 'px',
                  'top': topElmPos + $(body).scrollTop() - 80 + 'px'
                });

                if (scrollContainerElm) {
         

                  var scrollableContainer= $(scrollContainerElm);
...
var offsetHeight = offset.top + scrollableContainer.height();
            ...
            //Left
            if(((eventObj.pageX - offset.left) <= distance)
            ...
            //Right
            if((eventObj.pageX >= (offsetWidth - distance))
                    &&(eventObj.pageX < offsetWidth))
            {
                isMoving = true;
                clearInetervals();            
            ...
            //Top
            if(((eventObj.pageY - offset.top) <= distance)
                    &&(eventObj.pageY > offset.top))
            {
                isMoving = true;
                clearInetervals();            
                intTopHandler= setInterval(function(){
                    scrollableContainer.scrollTop(scrollableContainer.scrollTop() - step)
                },timer);
                console.log('top');                     
            }                          
            
            //Bottom
            if((eventObj.pageY >= (offsetHeight - distance))
                    &&(eventObj.pageY < offsetHeight))
            {
                isMoving = true;
                clearInetervals();            
            ...
            dragEnd = function (e) {
              var _targetElement = angular.element(e.target); // !kore added for smalltalk
              elements['droppedElement'] = _targetElement; // !kore added for smalltalk
              if(_targetElement){
                elements['droppedElementScope'] = _targetElement.scope();
              }
              var dragEventArgs = dragInfo.eventArgs(elements, pos); // !kore added for smalltalk
              ...
              element.bind('touchstart mousedown', function (e) {
                ...
                if (scope.dragDelay > 0) {
                  dragDelay.exec(function () {
                    $(".ontologyContainer").css({"cursor":"move"});
                    dragStartEvent(e);
                  }, scope.dragDelay);
                } else {
                  $(".ontologyContainer").css({"cursor":"move"});
              ...
              eventArgs: function (elements, pos) {
                return {
                  ...
                  dest: {
                    ...
                    destScope:elements['droppedElementScope'] // !kore added for smalltalk
                  },
                  elements: elements,
                  pos: pos
                };
