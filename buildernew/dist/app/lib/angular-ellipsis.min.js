	...
	return {
		...
		compile: function(elem, attr, linker) {
			...
			return function(scope, element, attributes) {
                ...
                var moreSpan = "";
                if(+scope.moreCount){
                    moreSpan = "<span class='ovalcountcircle'>+"+scope.moreCount+"</span>";
                }
				...
				function buildEllipsis() {
					...
					if (binding) {
						...
						if (isOverflowed(element, scope.useParent)) {
							...
							if (isHtml) {
								element.html(binding + appendString + ""+moreSpan);
							} else {
								element.text(binding).html(element.html() + appendString +""+moreSpan);
							}
							...
							for (; i < bindArrayStartingLength; i++) {
								...
								if (isHtml) {
									element.html(bindArray.join(ellipsisSeparator) + appendString+""+moreSpan);
								} else {
									element.text(bindArray.join(ellipsisSeparator)).html(element.html() + appendString+""+moreSpan);
								}
							...
							var _overflowed = element.attr('data-overflowed') || false;
							element.attr('data-overflowed', _overflowed);
                            element.html(element.html()+""+moreSpan);
						}
					}
				}
